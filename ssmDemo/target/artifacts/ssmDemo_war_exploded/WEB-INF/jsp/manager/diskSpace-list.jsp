...
<body>
				...
				<div class="panel-body">
					<div class="clearfix">
						<span class="f-l">
							...
							<a href="javascript:;" onclick="diskSpace_add('添加套餐','addDiskSpaceForm')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加套餐</a>
						</span>
					</div>
					<div class="mt-20 clearfix">
						<table id="diskSpaceTable" class="table table-border table-bordered table-bg table-hover table-sort">
							<thead>
								<tr>
	...
	<script type="text/javascript">
        $(document).ready(function() {
            //添加、修改异步提交地址
            var url = "diskSpaceList";

            diskSpaceTable=$("#diskSpaceTable").DataTable({
...
//全选,设置chheckbox name='all' tbody id=tb
            ...
            //表格选中行效果
            $('#diskSpaceTable tbody').on( 'click', 'tr', function (e) {
		...
		/*磁盘套餐-添加*/
        function diskSpace_add(title,url){
		...
		/*套餐状态更改*/
        $("#diskSpaceTable").find('tbody').on( 'click', '.btn_state', function () {
            var row = diskSpaceTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var title = '确定禁用该套餐？';
            var state = row.state;
            if(state == 0){
        ...
        $('#dataDels').click( function () {
            var length = diskSpaceTable.rows('.selected').data().length;
            if(length != 0 ){
                layer.confirm('确认要删除吗？',function(index){
                    var ids = [];
                    var rows = diskSpaceTable.rows('.selected').data();
                    ...
                    postData("delDiskSpace", data, function (result) {
                        if(result.status == "1"){
                            location.replace(location.href);
                        }else{
		...
		/*套餐-修改*/
        $("#diskSpaceTable").find('tbody').on( 'click', '.btn_update', function () {
            var row = diskSpaceTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var index = layer.open({
                type: 2,
                title: '套餐修改',
                content: 'updateDiskSpaceForm?id=' + id,
                area: ['800px', '400px']
            });
        });
	</script>

				...
				<div class="panel-body">
					<div class="clearfix">
						<span class="f-l">
							...
							<a href="javascript:;" onclick="admin_role_add('添加角色','addRoleForm')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加角色</a>
						</span>
					</div>
					<div class="mt-20 clearfix">
						<table id="roleTable" class="table table-border table-bordered table-bg table-hover table-sort">
							<thead>
								<tr>
...
//全选,设置chheckbox name='all' tbody id=tb
		...
		/*管理员-角色-删除*/
        $("#roleTable").find('tbody').on( 'click', '.btn_del', function () {
            var row = roleTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            layer.confirm('删除角色会将该角色下的所有管理员删除，确认要删除吗？',function(index){
                var ids = [];
				ids.push(id);
                ...
                postData("delRole", data, function (result) {
                    if(result.status == "1"){
        ...
        $('#dataDels').click( function () {
            var length = roleTable.rows('.selected').data().length;
            if(length != 0 ){
		...
		/*管理员-角色-修改*/
        $("#roleTable").find('tbody').on( 'click', '.btn_update', function () {
            var row = roleTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var index = layer.open({
                type: 2,
                title: '角色修改',
                content: 'updateRoleForm?roleid=' + id,
            });
            layer.full(index);
        });
	</script>

			...
			<div class="panel mt-20">
				<div class="panel-body">
					<div class="clearfix mt-20">
						<table id="appointmentTable" class="table table-border table-bordered table-hover table-bg table-sort">
							<thead>
								<tr class="text-c">
	...
	<script type="text/javascript">
        $(document).ready(function() {
            ...
            appointmentTable=$("#appointmentTable").DataTable({
                ...
                paging:true,
                lengthchange:true,
                pageLength: 9,  //首次加载的数据条数
                lengthMenu:[9,20,30,50],
...
//全选,设置chheckbox name='all' tbody id=tb
            ...
            //表格选中行效果
            $('#appointmentTable tbody').on( 'click', 'tr', function (e) {
		...
		/*预约服务-查看*/
        $("#appointmentTable").find('tbody').on( 'click', '.btn_check', function () {
            var row = appointmentTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;

            var index = layer.open({
                type: 2,
                title: '维权内容详情',
                content: 'checkForm?id=' + id,
                area: ['800px', '500px']
            });
        });

        /*预约服务-审核*/
        $("#appointmentTable").find('tbody').on( 'click', '.btn_examine', function () {
            var row = appointmentTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            layer.confirm('审核该申请？', {
                    btn: ['通过','不通过','取消'],
                ...
                function(){
                    ...
                    postData("appointmentExamine", data, function (result) {
                        if(result.status == "1"){

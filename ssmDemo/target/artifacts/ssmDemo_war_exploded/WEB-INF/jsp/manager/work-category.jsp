				...
				<div class="panel-body">
					<div class="clearfix">
						<span class="f-l">
							...
							<a href="javascript:;" onclick="cate_add('添加种类','addCateForm','800','700')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加种类</a>
						</span>
					</div>
					<div class="mt-20 clearfix">
						<table id="cateTable" class="table table-border table-bordered table-bg table-hover table-sort">
							<thead>
								<tr>
									<th scope="col" colspan="7">作品类别列表</th>
								</tr>
								<tr class="text-c">
									<th width="25"><input type="checkbox" value="" name="all"></th>
									<th width="15">ID</th>
									<th width="100">分类名称</th>
									<th width="100">类别代码</th>
									<th width="50">编码前缀代码</th>
...
//全选,设置chheckbox name='all' tbody id=tb
            ...
            //表格选中行效果
            $('#cateTable tbody').on( 'click', 'tr', function (e) {
		...
		/*作品管理-类别-删除*/
        $("#cateTable").find('tbody').on( 'click', '.btn_del', function () {
            var row = cateTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            layer.confirm('确认要删除吗？',function(index){
                ...
                postData("delCate", data, function (result) {
                    if(result.status == "1"){
        ...
        $('#dataDels').click( function () {
            var length = cateTable.rows('.selected').data().length;
            if(length != 0 ){
                layer.confirm('确认要删除吗？',function(index){
                    var ids = [];
                    var rows = cateTable.rows('.selected').data();
		...
		/*作品管理-类别-修改*/
        $("#cateTable").find('tbody').on( 'click', '.btn_update', function () {
            var row = cateTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var url = "updateCategoryForm?id=" + id;
            layer_show('修改类别',url,'800','700');
        });
	</script>
	<!--/请在上方写此页面业务相关的脚本-->
</body>

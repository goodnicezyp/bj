				...
				<form onsubmit="return false;" class="form form-horizontal" id="form-member-add">
					<div class="row clearfix">
						...
						<div class="form-controls col-xs-8 col-sm-9">
							<input type="text" class="input-text" value="${profess.name}" placeholder="职业名称" id="title" name="title">
						</div>
					</div>
					<div class="row cl">
							...
							<span class="select-box">
								<select class="select" size="1" name="demo1" id="cateOptions">
									...
									<c:forEach items="${cateList}" var="item">
										<option data-depth="${item.depth}" value="${item.id}" <c:if test="${profess.parentid == item.id}">selected</c:if> >${item.name}</option>
									</c:forEach>
								</select>
							</span>
						</div>
		...
		$(function(){
			...
			$("#form-member-add").validate({
				...
				submitHandler:function(form){
                    ...
                    postData("updateProfessional",data,function (res) {
                        if(res.status == "1"){

		...
		<div class="panel">
			<div class="panel-body">
				...
				<div class="row clearfix">
					...
					<div class="form-controls col-xs-8 col-sm-9">
						<input type="text" class="input-text" value="" placeholder="" id="rolecode" name="rolecode">
					</div>
				</div>
				<div class="row clearfix">
					...
					<div class="form-controls col-xs-8 col-sm-9">
						<c:forEach items="${permissionDtoList}" var="permission">
							<dl class="Hui-admin-permission-list">
								<dt>
									<label>
										<input type="checkbox" name="permissionInput" data-content="${permission.paction}">
											...
											<dd>
												<c:forEach items="${item.permissionList2}" var="item1">
													<c:if test="${fn:contains(item1.paction_3,'add')}">
														<label class="">
															<input type="checkbox" name="permissionInput" data-content="${item1.paction_3}" >添加
														</label>
													</c:if>
													<c:if test="${fn:contains(item1.paction_3,'update')}">
														<label class="">
															<input type="checkbox" name="permissionInput" data-content="${item1.paction_3}">
															修改
														</label>
													</c:if>
												...
												<label class="">
													<input type="checkbox" name="permissionInput" data-content="${item.paction_2}">
													查看</label>
											</dd>
										</dl>
									</c:forEach>
	...
	$(function(){
		...
		$("#form-admin-role-add").validate({
			...
			submitHandler:function(form){
				...
				postData("addRole",data,function (res) {
					if(res.status == "1"){

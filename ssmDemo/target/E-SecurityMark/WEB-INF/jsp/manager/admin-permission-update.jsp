    ...
    <form onsubmit="return false;" class="form form-horizontal" id="permissionAdd">
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" placeholder="权限名称" id="name" name="name" value="${permission.name}">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3">权限代码：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input id="action" name="action" type="text"  value="${permission.action}" class="input-text" placeholder="格式(全英文)：'权限所属功能模块_权限操作'，如：admin_add">
            </div>
        </div>
        <div class="row cl">
                ...
                <span class="select-box">
                    <select class="select" size="1" name="demo1" id="permissionOptions">
                        ...
                        <c:forEach items="${permissionList}" var="perm">
                            <option data-depth="${perm.depth}" value="${perm.pid}" <c:if test="${permission.parentID == perm.pid}">selected="selected"</c:if>>${perm.name}</option>
                        </c:forEach>
                    </select>
                </span>
            </div>
...
<script type="text/javascript">
    $("#permissionAdd").validate({
        ...
        submitHandler:function(form){
            ...
            postData("updatePermission",data,function (res) {
                if(res.status == "1"){
    ...
    $("#permissionOptions").change(function () {
        var selectID = $(this).find("option:selected").val();
        var curID="${permission.pid}";
        if(curID == selectID){
            $(this).val("${permission.parentID}");
            layer.msg("不能选择自身作为父类",{icon:2,time:2000}) //icon:  1:成功  2：失败
        }
    })
</script>

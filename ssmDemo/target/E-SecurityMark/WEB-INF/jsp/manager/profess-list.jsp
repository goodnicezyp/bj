...
<body>
				...
				<div class="panel-body">
					<div class="clearfix">
						<span class="f-l">
							...
							<a href="javascript:;" onclick="profess_add('添加职业','addProfessionalForm')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加职业</a>
						</span>
					</div>
					<div class="mt-20 clearfix">
						<table id="professTable" class="table table-border table-bordered table-bg table-hover table-sort">
							<thead>
								<tr>
									<th scope="col" colspan="8">职业列表</th>
								</tr>
								<tr class="text-c">
									<th width="25"><input type="checkbox" value="all" name="all"></th>
									<th width="40">ID</th>
									<th>职业名称</th>
									<th>父类名称</th>
									<th>状态</th>
	...
	<script type="text/javascript">
        $(document).ready(function() {
            ...
            professTable=$("#professTable").DataTable({
...
//全选,设置chheckbox name='all' tbody id=tb
            ...
            //表格选中行效果
            $('#professTable tbody').on( 'click', 'tr', function (e) {
		...
		/*-添加*/
        function profess_add(title,url){
		...
		/*职业状态更改*/
        $("#professTable").find('tbody').on( 'click', '.btn_state', function () {
            var row = professTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var title = '确定禁用该职业？';
            var state = row.state;
            if(state == 0){
        ...
        $('#dataDels').click( function () {
            var length = professTable.rows('.selected').data().length;
            if(length != 0 ){
                layer.confirm('确认要删除吗？',function(index){
                    var ids = [];
                    var rows = professTable.rows('.selected').data();
                    ...
                    postData("delProfessional", data, function (result) {
                        if(result.status == "1"){
                            location.replace(location.href);
                        }else{
		...
		/*-修改*/
        $("#professTable").find('tbody').on( 'click', '.btn_update', function () {
            var row = professTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var index = layer.open({
                type: 2,
                title: '职业修改',
                content: 'updateProfessionalForm?id=' + id,
                area: ['600px', '300px']
            });
        });
	</script>

							...
							<span class="select-box" style="width:150px;">
								<select class="select" name="monthNum" size="1" id="monthNum">
									...
									<c:forEach begin="1" end="${num}" varStatus="idx">
										<option <c:if test="${idx.index == 1}"> checked</c:if> value="${idx.index}">${idx.index}</option>
									</c:forEach>
								</select>
							</span>
						</div>
						...
						<div class="form-controls col-xs-8 col-sm-9">
							<span class="select-box" style="width:150px;">
								<select class="select" name="memberid" size="1" id="memberid">
									<option value="1">会员1</option>
									<option value="2">会员2</option>
								</select>
							</span>
						</div>
					</div>
		...
		$(function(){
			...
			$("#form-member-add").validate({
				...
				submitHandler:function(form){
                    ...
                    postData("addMemberPacket",data,function (res) {
                        if(res.status == "1"){

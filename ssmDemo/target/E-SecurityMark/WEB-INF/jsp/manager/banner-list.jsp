				...
				<div class="panel-body">
					<div class="clearfix">
						<span class="f-l">
							...
							<a href="javascript:;" onclick="banner_add('添加banner图','addBannerForm')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
						</span>
					</div>
					<div class="mt-20 clearfix">
						<table id="bannerTable" class="table table-border table-bordered table-bg table-hover table-sort">
							<thead>
								<tr>
									<th scope="col" colspan="10">banner列表</th>
								</tr>
								<tr class="text-c">
            ...
            bannerTable=$("#bannerTable").DataTable({
                ...
                columnDefs: [
                    ...
                    { targets: -3,render:function (val) {
                        if (val == 1) {
                            return "<span class='label label-primary radius'>首页banner</span>";
                        }else{
                            return "<span class='label label-success radius'>最新活动</span>";
                        }
                    }},
                    { targets: -2,render:function (val) {
                        if (val == 1) {
...
//全选,设置chheckbox name='all' tbody id=tb
            ...
            //表格选中行效果
            $('#bannerTable tbody').on( 'click', 'tr', function (e) {
		...
		/*套餐状态更改*/
        $("#bannerTable").find('tbody').on( 'click', '.btn_state', function () {
            var row = bannerTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var title = '确定禁用该套餐？';
            var state = row.flag;
            if(state == 0){
        ...
        $('#dataDels').click( function () {
            var length = bannerTable.rows('.selected').data().length;
            if(length != 0 ){
                layer.confirm('确认要删除吗？',function(index){
                    var ids = [];
                    var rows = bannerTable.rows('.selected').data();
                    ...
                    postData("delBanner", data, function (result) {
                        if(result.status == "1"){
                            location.replace(location.href);
                        }else{
		...
		/*套餐-修改*/
        $("#bannerTable").find('tbody').on( 'click', '.btn_update', function () {
            var row = bannerTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var index = layer.open({
                type: 2,
                title: '套餐修改',
                content: 'updateBannerForm?id=' + id,
                area: ['800px', '750px']
            });
        });
	</script>

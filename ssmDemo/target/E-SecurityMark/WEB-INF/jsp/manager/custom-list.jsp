...
<body>
				...
				<div class="panel-body">
					<div class="clearfix">
						<span class="f-l">
							...
							<a href="javascript:;" onclick="custom_add('添加客服','addCustomForm')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加客服</a>
						</span>
					</div>
					<div class="mt-20 clearfix">
						<table id="customTable" class="table table-border table-bordered table-bg table-hover table-sort">
							<thead>
								<tr>
									<th scope="col" colspan="8">客服列表</th>
								</tr>
								<tr class="text-c">
	...
	<script type="text/javascript">
        $(document).ready(function() {
            ...
            customTable=$("#customTable").DataTable({
...
//全选,设置chheckbox name='all' tbody id=tb
            ...
            //表格选中行效果
            $('#customTable tbody').on( 'click', 'tr', function (e) {
		...
		/*客服状态更改*/
        $("#customTable").find('tbody').on( 'click', '.btn_state', function () {
            var row = customTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var title = '确定禁用该客服？';
            var state = row.state;
            if(state == 0){
        ...
        $('#dataDels').click( function () {
            var length = customTable.rows('.selected').data().length;
            if(length != 0 ){
                layer.confirm('确认要删除吗？',function(index){
                    var ids = [];
                    var rows = customTable.rows('.selected').data();
                    ...
                    postData("delCustom", data, function (result) {
                        if(result.status == "1"){
                            location.replace(location.href);
                        }else{
		...
		/*客服-修改*/
        $("#customTable").find('tbody').on( 'click', '.btn_update', function () {
            var row = customTable.row($(this).closest('tr')).data(); //这个插件要取到值还真是只有这样的绑定事件才可以 原因不明
            var id = row.id;
            var index = layer.open({
                type: 2,
                title: '客服修改',
                content: 'updateCustomForm?id=' + id,
                area: ['800px', '500px']
            });
        });
	</script>
